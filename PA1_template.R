...
png('totalStepsPerDayWithMeanMedian_Hist.png')
...
png('AverageDailyActivityPattern.png')
...
png('totalStepsPerDayWithImputedMissingValues_Hist.png')
hist(steps_sum$steps, main = "Histogram of total steps taken per day", 
     xlab = "Total steps per day", ylab = "Number of days", 
     breaks = 10, col = "steel blue")
abline(v = mean(steps_sum$steps), lty = 1, lwd = 2, col = "red")
...
library(ggplot2)
png('StepsMeanInterval.png')
g <- ggplot(stepsmean_interval, aes(interval, steps)) + geom_line() + facet_grid(temp ~ .) 
print(g)
dev.off()